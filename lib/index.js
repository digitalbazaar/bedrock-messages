...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  // Get endpoint
  app.get(config.messages.endpoints.messages, rest.when.prefers.ld,
    ensureAuthenticated, function(req, res, next) {
...
function get(actor, query, recipient, callback) {
  ...
  async.auto({
    ...
    totalCount: ['checkPermissions', function(callback) {
      var q = {
        recipient: database.hash(recipient)
      };
      store.count(q, callback);
    }],
    queryCount: ['checkPermissions', function(callback) {
      var q = {
        recipient: database.hash(recipient)
      };
      var options = {
