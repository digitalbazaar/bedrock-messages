...
function getUnbatchedMessage(batch, done) {
  ...
  if(batch) {
    ...
    return store.findOne(q, function(err, result) {
      ...
      if(result) {
        return resetMessage(batch, result.value, function(err, result) {
          if(err) {
            return callback(err);
          }
          if(!result) {
            return loop(null, done);
          }
          done(null, result);
        });
      }
  ...
  async.auto({
    ...
    if(results.readBatch && !results.resetMessage) {
      return loop(results.readBatch, done);
    }
