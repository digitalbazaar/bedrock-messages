      ...
      getBatch: function(callback) {
        ...
        async.each(uniqueRecipients, function(recipient, callback) {
          readBatch(recipient, function(err, result) {
            ...
            batchIds[recipient] = result.value.value.id;
            callback();
          });
...
api._batchMessage = function(callback) {
  batchMessage(callback);
};
...
function batchMessage(callback) {
  var done = false;
  async.until(function() {
    ...
    async.auto({
      getPendingMessage: function(callback) {
        var q = {
          'value.meta.batch.state': 'pending'
        };
        store.findOne(q, callback);
      },
      updateBatch: ['getPendingMessage', function(callback, results) {
        if(!results.getPendingMessage) {
          done = true;
          return callback();
        }
        var message = results.getPendingMessage.value;
        var q = {
      ...
      updateMessage: ['updateBatch', function(callback, results) {
        ...
        var q = {
          'value.meta.batch.state': 'pending',
          'value.meta.batch.id': results.getPendingMessage.value.meta.batch.id
        };
        var u = {
          $set: {
      ...
      removeFromMap: ['updateMessage', function(callback, results) {
        if(results.updateMessage.modifiedCount === 0) {
          // batchId on the message changed or another process changed
          return callback();
        }
