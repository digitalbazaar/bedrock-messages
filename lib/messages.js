  ...
  async.forEachOf(messageCollection, function(collection, key, callback) {
    ...
    async.auto({
      ...
      batch: ['store', function(callback, results) {
        ...
        // console.log('RESULTSPROCESS$$$$$$$$$$', results.process);
        callback();
      }]
    }, callback);
  }, function(err) {
...
function getUnbatchedMessage(batch, callback) {
  var q;
  if(batch) {
    ...
    return store.findOne(q, function(err, results) {
      if(err) {
        return callback(err);
      }
      callback(null, {batch: batch, message: results.value});
    });
  }
  async.auto({
      ...
      storeBatch.findOne(q, function(err, result) {
        if(result) {
          return process.nextTick(function() {
            getUnbatchedMessage(result.value, callback);
          });
        }
        callback(err, result);
      });
    ...
    if(results.findDirty) {
      return callback(null, results.findDirty);
    }
    if(results.findPending) {
      var batch = results.readBatch;
      var message = results.findPending;
      return callback(null, {batch: batch, message: message});
    }
    callback();
  });
}
