...
function update(request, callback) {
  ...
  if(request.operation === 'archive') {
    ...
    updateResults.error = new BedrockError(request.operation + ' is not a suitable update operation');
...
function batchUpdate(request, callback) {
  if (request.operation === 'archive') {
    ...
    var error = new BedrockError('No suitable update operation', 'MessageUpdateBatch', {
        httpStatusCode: 400,
        public: true,
        body: updateResults
    });
  ...
  store.updateMany(query, update, function(err, results) {
    ...
    if(err) {
      updateResults.error = err;
      error = new BedrockError('Database batch update failed', 'MessageUpdateBatch', {
        httpStatusCode: 500,
        public: true,
        body: updateResults
      });
...
function del(id, callback) {
  ...
  store.deleteOne(query, function(err, results) {
    ...
    if (err) {
      ...
      error = new BedrockError('Database message delete failed', 'MessageDelete', {
        httpStatusCode: 500,
        public: true,
        body: deleteResults
      });
...
function batchDelete(request, callback) {
  ...
  store.deleteMany(query, function(err, results) {
    ...
    if (err) {
      deleteResults.error = err;
      error = new BedrockError('Database message batch delete failed', 'MessageDeleteBatch', {
        httpStatusCode: 500,
        public: true,
        body: deleteResults
      });
