...
config.scheduler.jobs.push({
  id: 'bedrock-messages.jobs.cleanupJob',
  type: 'bedrock-messages.jobs.cleanupJob',
  schedule: config.messages.scheduler.frequency,
  priority: config.messages.scheduler.priority,
  concurrency: config.messages.scheduler.concurrency
});

bedrock.events.on('bedrock.init', function() {
  scheduler.define('bedrock-messages.jobs.cleanUpJob', function(job, callback) {
    cleanUpJob(callback);
  });
});
...
function getNew(actor, recipient, callback) {
  ...
  async.auto({
    ...
    if(closeBatchResult != null) {
      messages = closeBatchResult.messages;
    }
    callback(null, messages);
  });
}
