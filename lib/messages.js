...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  // Archive endpoint
  app.post(
    config.messages.endpoints.messagesArchive, rest.when.prefers.ld,
    ensureAuthenticated, function(req, res, next) {
      batchArchive(req.body, function(err, results) {
        // FIXME: address error conditions, and should we return a response similar to get?
        res.json('');
      });
    });
  // Delete endpoint
  app.post(
    config.messages.endpoints.messagesDelete, rest.when.prefers.ld,
    ensureAuthenticated, function(req, res, next) {
      //console.log(req);
...
function batchArchive(messages, callback) {
  var update = {
    $set: {
      'value.meta.archived': true
    }
  };
  // TODO: is there a batch command to do this?
  for(var message in messages) {
    var query = {
      id: database.hash(messages[message].id)
    };
    // FIXME: should a message being archived be an 'event' as well?
    store.update(query, update, []);
  }
  // TODO: Error handling, and should we return a response similar to get?
  callback(null, null);
}

function batchDelete(messages, callback) {
  // TODO: is there a batch command to do this?
  for(var message in messages) {
    var query = {
      id: database.hash(messages[message].id)
