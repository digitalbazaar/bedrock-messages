...
function batchUpdate(requests, callback) {
  async.each(requests, function(request, asyncCallback) {
      var update;
      if (request.operation === 'archive') {
        update = {
          $set: {
    ...
    function(err) {
      // Making use of asyncCallback's optional error paramater breaks async, so we can't
      // take advantage of it unfortunately
      var error = null;
      for(var request in requests) {
        if(requests[request].error) {
...
function batchDelete(requests, callback) {
  var ids = requests.map(function(request) {
    return database.hash(request.message.id);
  });
  var query = {
    id: {$in: ids}
  }
  store.deleteMany(query, function(err, results) {
    requests.result = results.result;
