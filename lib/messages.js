...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  app.delete(
    config.messages.endpoints.messages + '/:id', rest.when.prefers.ld,
    ensureAuthenticated, function(req, res, next) {
      deleteMessage(req.params.id, function(err, results) {
        if(err) {
          return next(err);
        }
        res.json(results);
...
function batchUpdate(actor, request, options, callback) {
  ...
  async.auto({
    ...
    batchOperation: ['checkPermissions', function(callback) {
      ...
      var u = {$set: {'value.meta.archived': true}};
      store.updateMany(q, u, function(err, results) {
        var updateResults = {};
        updateResults.result = results.result;
        updateResults.operation = request.operation;
    ...
    callback(null, results.batchOperation);
  });
}
...
function deleteMessage(actor, id, options, callback) {
  async.auto({
    ...
    callback(null, results.deleteOperation);
  });
}
