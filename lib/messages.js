...
function batchMessage(batch, message, callback) {
  async.auto({
    updateBatch: [function(callback, results) {
      var q = {
      ...
      storeBatch.updateOne(q, u, callback);
    }],
...
function resetMessage(batch, message, done) {
  ...
  async.auto({
    ...
    process: ['updateMessage', function(callback, results) {
      // FIXME: if updateMessage matches but does not modify
      // we can go into infinite loop
      if(results.updateMessage.modifiedCount === 0) {
        return process.nextTick(function() {
          getUnbatchedMessage(batch, done);
        });
      }
      callback(null, {batch: batch, message: message});
    }]
  }, function(err, results) {
    done(err, results.process);
  });
}
  ...
  if(batch) {
    ...
    return store.findOne(q, function(err, result) {
      ...
      process.nextTick(function() {
        // TODO: should mark batch clean
        // no match found in batch, query for new batch
        async.auto({
          resetBatch: function(callback) {
            resetBatch(batch, callback);
          },
          getUnbatched: ['resetBatch', function(callback) {
            // FIXME: this can go into loop under the right conditions
            console.log('LOOPING_AAAA');
            getUnbatchedMessage(null, callback);
          }]
        }, function(err, results) {
          done(err, results.getUnbatched);
        });
      });
    });
  }
  ...
  async.auto({
    findDirty: function(callback) {
      ...
      storeBatch.findOne(q, function(err, result) {
        if(result) {
          return process.nextTick(function() {
            getUnbatchedMessage(result.value, done);
          });
        }
        callback(err, result);
      });
    },
    findPending: ['findDirty', function(callback, results) {
