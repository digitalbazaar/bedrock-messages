...
api._deliverBatch = function(callback) {
  deliverBatch(callback);
};
...
function deliverBatch(callback) {
  var done = false;
  async.until(function() {
    return done;
  }, function(callback) {
    // FIXME: added dirty checking to query to prevent infinite loop
    var q = {
      'value.messages': {$ne: {}},
      $or: [{'value.dirty': false}, {'value.dirty': {$exists: false}}]
    };
    var u = {
      $inc: {'value.id': 1},
      $set: {'value.dirty': true}
    };
    storeBatch.findAndModify(
      q, [], u, database.writeOptions, function(err, result) {
      if(err) {
        return callback(err);
      }
      if(!result.value) {
        ...
        var u = {
          $inc: {'value.id': 1},
          $set: {'value.dirty': false}
        };
        return storeBatch.findAndModify(
          q, [], u, database.writeOptions, function(err, result) {
          if(err) {
            return callback(err);
          }
